...
# install packages from packages_list.txt with remotes
...
for (i in seq_along(p$V1))
    ...
    if(!require(p$V1[i], character.only=TRUE))
        ...
        if (grepl('local:', p$V1[i]))  # local package
            {
            package_file_name = strsplit(p$V1[i], ':')[[1]][2]        # -> foo_8.0.0.tar.gz foo
            package_file = strsplit(package_file_name, ' ')[[1]][1]   # -> foo_8.0.0.tar.gz
            package_name = strsplit(package_file_name, ' ')[[1]][2]   # -> foo
            ...
            package_file_name = strsplit(p$V1[i], ':')[[1]][2]         # foo/bar bar
            package_file = strsplit(package_file_name, ' ')[[1]][1]    # foo/bar
            package_name = strsplit(package_file_name, ' ')[[1]][2]    # bar
            ...
            package_file = strsplit(package_file_name, ' ')[[1]][1]   # foo
            package_name = strsplit(package_file_name, ' ')[[1]][2]   # foo 
